...
pub struct ProgramSpec {
    ...
    /// Complete classification of implicit runtime input slots whenever
    ...
    pub node_provenance: Vec<NodeProvenanceEntrySpec>,
}

/// Runtime-input classification supplied by the composition author.
///
/// A compact enum prevents invalid state/input-kind pairs: `exogenous` and
/// `policy_derived` compile to state `input` with the matching input kind;
/// `pending` compiles to state `pending` with no input kind.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
#[serde(rename_all = "snake_case")]
pub enum InputStateSpec {
    Exogenous,
    PolicyDerived,
    Pending,
}

/// Executable node kinds used by the provenance sidecar and compiled catalog.
...
/// Whether an executable declaration is grounded in a cited provision,
/// generated by composition, or cannot be classified from the legacy source.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
#[serde(rename_all = "snake_case")]
pub enum NodeProvenanceSpec {
    ProvisionBacked,
    Synthesized,
    Unverified,
}

/// Provenance for one declared executable node before reachability is derived.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
pub struct NodeProvenanceEntrySpec {
